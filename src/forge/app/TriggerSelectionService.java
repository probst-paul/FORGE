...
public class TriggerSelectionService {
    ...
    public Class<? extends TradeTrigger> selectTrigger(UserInput input, UserOutput output) {
        List<Class<? extends TradeTrigger>> triggers = facadeTrigger.findAvailableTriggers();
        if (triggers.isEmpty()) {
        ...
        for (int i = 0; i < triggers.size(); i++) {
            output.printLine((i + 1) + ". " + facadeTrigger.getDisplayName(triggers.get(i)));
        }
    ...
    public String getDisplayName(Class<? extends TradeTrigger> trigger) {
        return facadeTrigger.getDisplayName(trigger);
    }
}
